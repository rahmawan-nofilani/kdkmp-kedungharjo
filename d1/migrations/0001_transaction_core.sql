-- KDKMP Kedungharjo — D1 transactional core
-- Development schema. All monetary amounts are stored as integer rupiah units.
-- Supabase remains the identity / access / member-control plane.
...
CREATE INDEX IF NOT EXISTS inventory_movements_stock_idx
  ON inventory_movements (organization_id, warehouse_id, product_id, occurred_at);
...
CREATE INDEX IF NOT EXISTS inventory_movements_expiry_idx
  ON inventory_movements (organization_id, expiry_date)
...
CREATE INDEX IF NOT EXISTS teller_shifts_user_status_idx
  ON teller_shifts (organization_id, teller_user_id, status, opened_at DESC);
...
CREATE INDEX IF NOT EXISTS transaction_audit_entity_idx
  ON transaction_audit_events (organization_id, entity_type, entity_id, created_at DESC);

-- IMPORTANT: current stock is derived from SUM(inventory_movements.quantity_delta).
-- Do not add an editable `stock` column to products.
-- IMPORTANT: a sale is considered valid only when the application transaction commits
-- sale + lines + payment + inventory movements + balanced journal entries atomically.
