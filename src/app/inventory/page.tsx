...
function movementLabel(type:string){const labels:Record<string,string>={OPENING:"Opening",PURCHASE_RECEIPT:"Penerimaan",SALE:"Penjualan",SALE_VOID:"Void masuk",ADJUSTMENT_IN:"Adjustment +",ADJUSTMENT_OUT:"Adjustment −",TRANSFER_IN:"Transfer masuk",TRANSFER_OUT:"Transfer keluar"};return labels[type]??type}
...
export default async function InventoryPage({searchParams}:PageProps){
 ...
 return <PageContainer size="full">
  <PageHeader eyebrow="Operasional · Inventory" title="Stok & Gudang" description="Stok dihitung dari inventory ledger. Opening, receiving, sale, void, transfer, dan adjustment tetap meninggalkan movement serta audit trail." actions={<Link className={styles.primaryButton} href="/inventory/opname">Stock Opname</Link>}/>
  ...
  {params.status==="stock-posted"?<Alert tone="success" title="Opening stock diposting">Saldo awal masuk ke inventory ledger.</Alert>:null}
  {params.status==="stock-adjusted"?<Alert tone="success" title="Adjustment diposting">Movement koreksi dan audit trail berhasil dibuat.</Alert>:null}
  ...
  <section className={styles.metrics}>
   ...
   <Card density="compact"><span>Gudang</span><strong>{warehouses.length}</strong><small>lokasi aktif/nonaktif</small></Card>
   <Card density="compact"><span>Total unit stok</span><strong>{stockUnits}</strong><small>dari movement ledger</small></Card>
