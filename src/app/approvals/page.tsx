...
export default async function ApprovalCenterPage(){
  ...
  const canOpen=["APPROVAL_VIEW","PURCHASE_APPROVE","INVOICE_APPROVE","JOURNAL_APPROVE","ASSET_APPROVE","SAVINGS_PRODUCT_APPROVE","SAVINGS_ACCOUNT_APPROVE","LOAN_PRODUCT_APPROVE","LOAN_APPLICATION_APPROVE","ORG_MANAGE"].some(code=>allowed.has(code));
  ...
  const [d1Queue,savingsProductResult,savingsAccountResult,loanProductResult,loanApplicationResult]=await Promise.all([getApprovalQueue(access.organization.id),savingsProductTask,savingsAccountTask,loanProductTask,loanApplicationTask]);
  ...
  const allQueue:QueueItem[]=[...(d1Queue as QueueItem[]),...savingsProductQueue,...savingsAccountQueue,...loanProductQueue,...loanApplicationQueue].sort((a,b)=>a.createdAt.localeCompare(b.createdAt));
  ...
  return <main className={styles.page}><header className={styles.topbar}><div><p>KONTROL · PEMERIKSAAN</p><h1>Pusat Persetujuan</h1></div><nav><Link href="/dashboard">Dashboard</Link></nav></header><div className={styles.content}>
    ...
    <section className={styles.notice}><strong>Kenapa tidak ada tombol “Setujui Semua”?</strong><p>Karena setiap keputusan membutuhkan konteks. Produk, rekening simpanan, dan pengajuan pinjaman tetap harus dibuka serta diperiksa sebelum disetujui.</p></section>
  </div></main>;
}
