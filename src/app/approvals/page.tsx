...
export default async function ApprovalCenterPage(){
  ...
  const canOpen=["APPROVAL_VIEW","PURCHASE_APPROVE","INVOICE_APPROVE","JOURNAL_APPROVE","ASSET_APPROVE","SAVINGS_PRODUCT_APPROVE","SAVINGS_ACCOUNT_APPROVE","LOAN_PRODUCT_APPROVE","LOAN_APPLICATION_APPROVE","LOAN_DISBURSEMENT_APPROVE","LOAN_PENALTY_WAIVE_APPROVE","ORG_MANAGE"].some(code=>allowed.has(code));
  ...
  const loanProductTask=(allowed.has("LOAN_PRODUCT_APPROVE")||allowed.has("APPROVAL_VIEW"))
    ...
    :Promise.resolve({data:[],error:null});
  const loanApplicationTask=(allowed.has("LOAN_APPLICATION_APPROVE")||allowed.has("APPROVAL_VIEW"))
    ?supabase.from("loan_applications").select("id,application_number,status,requested_principal_amount,requested_tenor_months,created_by,submitted_at,product_snapshot").eq("organization_id",access.organization.id).in("status",["SUBMITTED","UNDER_REVIEW"]).order("submitted_at",{ascending:true}).limit(80)
    :Promise.resolve({data:[],error:null});
  const loanDisbursementTask=(allowed.has("LOAN_DISBURSEMENT_APPROVE")||allowed.has("APPROVAL_VIEW"))
    ?supabase.from("loan_disbursements").select("id,disbursement_number,channel,recipient_name,net_disbursement_amount,created_by,created_at,submitted_at").eq("organization_id",access.organization.id).eq("status","SUBMITTED").order("submitted_at",{ascending:true}).limit(80)
    :Promise.resolve({data:[],error:null});
  const penaltyWaiverTask=(allowed.has("LOAN_PENALTY_WAIVE_APPROVE")||allowed.has("APPROVAL_VIEW"))
    ?supabase.from("loan_penalty_waivers").select("id,waiver_number,contract_id,requested_amount,reason,created_by,created_at,submitted_at").eq("organization_id",access.organization.id).eq("status","SUBMITTED").order("submitted_at",{ascending:true}).limit(80)
    :Promise.resolve({data:[],error:null});

  const [d1Queue,savingsProductResult,savingsAccountResult,loanProductResult,loanApplicationResult,loanDisbursementResult,penaltyWaiverResult]=await Promise.all([getApprovalQueue(access.organization.id),savingsProductTask,savingsAccountTask,loanProductTask,loanApplicationTask,loanDisbursementTask,penaltyWaiverTask]);
  ...
  const loanApplicationQueue:QueueItem[]=(loanApplicationResult.data??[]).map((row:any)=>{const snapshot=(row.product_snapshot&&typeof row.product_snapshot==="object")?row.product_snapshot:{};return {key:`loan-application:${row.id}`,category:"PENGAJUAN_PINJAMAN",reference:row.application_number,title:String(snapshot.display_name||"Pengajuan Pinjaman"),detail:row.status==="UNDER_REVIEW"?"Pemeriksaan sudah dimulai dan menunggu keputusan checker.":"Pengajuan telah lulus eligibility gate dan menunggu checker yang berbeda.",amount:Number(row.requested_principal_amount||0),createdBy:row.created_by,createdAt:row.submitted_at,href:`/loans/applications/${row.id}`};});
  const loanDisbursementQueue:QueueItem[]=(loanDisbursementResult.data??[]).map((row:any)=>({key:`loan-disbursement:${row.id}`,category:"PENCAIRAN_PINJAMAN",reference:row.disbursement_number,title:`${row.channel} · ${row.recipient_name}`,detail:"Permintaan pencairan sudah diajukan. Checker harus memastikan sumber dana, tujuan, kanal, dan nominal sebelum menyetujui.",amount:Number(row.net_disbursement_amount||0),createdBy:row.created_by,createdAt:row.submitted_at||row.created_at,href:`/loans/disbursements/${row.id}`}));
  const penaltyWaiverQueue:QueueItem[]=(penaltyWaiverResult.data??[]).map((row:any)=>({key:`penalty-waiver:${row.id}`,category:"WAIVER_DENDA",reference:row.waiver_number,title:"Permohonan pembebasan denda pinjaman",detail:String(row.reason||"Waiver denda menunggu checker berbeda."),amount:Number(row.requested_amount||0),createdBy:row.created_by,createdAt:row.submitted_at||row.created_at,href:`/loans/penalties#waiver-${row.id}`}));
  const allQueue:QueueItem[]=[...(d1Queue as QueueItem[]),...savingsProductQueue,...savingsAccountQueue,...loanProductQueue,...loanApplicationQueue,...loanDisbursementQueue,...penaltyWaiverQueue].sort((a,b)=>a.createdAt.localeCompare(b.createdAt));
  ...
  return <main className={styles.page}><header className={styles.topbar}><div><p>KONTROL · PEMERIKSAAN</p><h1>Pusat Persetujuan</h1></div><nav><Link href="/dashboard">Dashboard</Link></nav></header><div className={styles.content}>
    ...
    <section className={styles.notice}><strong>Kenapa tidak ada tombol “Setujui Semua”?</strong><p>Karena setiap keputusan membutuhkan konteks. Produk, rekening, pengajuan/pencairan pinjaman, dan waiver denda harus dibuka serta diperiksa sebelum disetujui.</p></section>
  </div></main>;
}
