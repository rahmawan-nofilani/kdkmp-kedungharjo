...
function categoryLabel(category:string){return ({PEMBELIAN:"Pembelian",HUTANG_SUPPLIER:"Hutang Pemasok",JURNAL:"Jurnal",ASET:"Aset Tetap",PENYUSUTAN:"Penyusutan",STOCK_OPNAME:"Hitung Stok",PRODUK_SIMPANAN:"Produk Simpanan",REKENING_SIMPANAN:"Rekening Simpanan"} as Record<string,string>)[category]||category;}
function canReviewCategory(category:string,p:Set<string>){if(p.has("APPROVAL_VIEW"))return true;if(category==="PEMBELIAN")return p.has("PURCHASE_APPROVE");if(category==="HUTANG_SUPPLIER")return p.has("INVOICE_APPROVE");if(category==="JURNAL")return p.has("JOURNAL_APPROVE");if(category==="ASET"||category==="PENYUSUTAN")return p.has("ASSET_APPROVE");if(category==="STOCK_OPNAME")return p.has("ORG_MANAGE");if(category==="PRODUK_SIMPANAN")return p.has("SAVINGS_PRODUCT_APPROVE");if(category==="REKENING_SIMPANAN")return p.has("SAVINGS_ACCOUNT_APPROVE");return false;}
...
export default async function ApprovalCenterPage(){
  ...
  const canOpen=["APPROVAL_VIEW","PURCHASE_APPROVE","INVOICE_APPROVE","JOURNAL_APPROVE","ASSET_APPROVE","SAVINGS_PRODUCT_APPROVE","SAVINGS_ACCOUNT_APPROVE","ORG_MANAGE"].some(code=>allowed.has(code));
  ...
  const [d1Queue,savingsProductResult,savingsAccountResult]=await Promise.all([getApprovalQueue(access.organization.id),savingsProductTask,savingsAccountTask]);
  ...
  const allQueue:QueueItem[]=[...(d1Queue as QueueItem[]),...savingsProductQueue,...savingsAccountQueue].sort((a,b)=>a.createdAt.localeCompare(b.createdAt));
  ...
  return <main className={styles.page}><header className={styles.topbar}><div><p>KONTROL · PEMERIKSAAN</p><h1>Pusat Persetujuan</h1></div><nav><Link href="/dashboard">Dashboard</Link></nav></header><div className={styles.content}>
    ...
    <section className={styles.notice}><strong>Kenapa tidak ada tombol “Setujui Semua”?</strong><p>Karena setiap keputusan membutuhkan konteks. Produk dan rekening simpanan tetap harus dibuka serta diperiksa sebelum diaktifkan.</p></section>
  </div></main>;
}
