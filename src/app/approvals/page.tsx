...
function categoryLabel(category:string){return ({PEMBELIAN:"Pembelian",HUTANG_SUPPLIER:"Hutang Pemasok",JURNAL:"Jurnal",ASET:"Aset Tetap",PENYUSUTAN:"Penyusutan",STOCK_OPNAME:"Hitung Stok",PRODUK_SIMPANAN:"Produk Simpanan"} as Record<string,string>)[category]||category;}
function canReviewCategory(category:string,p:Set<string>){if(p.has("APPROVAL_VIEW"))return true;if(category==="PEMBELIAN")return p.has("PURCHASE_APPROVE");if(category==="HUTANG_SUPPLIER")return p.has("INVOICE_APPROVE");if(category==="JURNAL")return p.has("JOURNAL_APPROVE");if(category==="ASET"||category==="PENYUSUTAN")return p.has("ASSET_APPROVE");if(category==="STOCK_OPNAME")return p.has("ORG_MANAGE");if(category==="PRODUK_SIMPANAN")return p.has("SAVINGS_PRODUCT_APPROVE");return false;}
...
export default async function ApprovalCenterPage(){
  ...
  const canOpen=["APPROVAL_VIEW","PURCHASE_APPROVE","INVOICE_APPROVE","JOURNAL_APPROVE","ASSET_APPROVE","SAVINGS_PRODUCT_APPROVE","ORG_MANAGE"].some(code=>allowed.has(code));
  ...
  const savingsTask=(allowed.has("SAVINGS_PRODUCT_APPROVE")||allowed.has("APPROVAL_VIEW"))
  ...
  return <main className={styles.page}><header className={styles.topbar}><div><p>KONTROL · PEMERIKSAAN</p><h1>Pusat Persetujuan</h1></div><nav><Link href="/dashboard">Dashboard</Link></nav></header><div className={styles.content}>
    ...
    <section className={styles.notice}><strong>Kenapa tidak ada tombol “Setujui Semua”?</strong><p>Karena setiap keputusan perlu konteks. Produk simpanan harus dibaca aturan setor, tarik, saldo minimum, masa kunci, dan penggunaan saldo sebelum versi diaktifkan.</p></section>
  </div></main>;
}
