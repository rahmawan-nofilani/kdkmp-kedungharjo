...
type Product={id:string;sku:string;barcode:string|null;name:string;unit_name:string;sell_amount:number;stock_qty:number;track_stock:number};
...
const ProductButton=memo(function ProductButton({product,onAdd}:{product:Product;onAdd:(product:Product)=>void}){
  const unavailable=Boolean(product.track_stock&&product.stock_qty<=0);
  return <ProductCard sku={product.sku} name={product.name} price={product.sell_amount} stockQty={product.stock_qty} unitName={product.unit_name} trackStock={Boolean(product.track_stock)} onClick={()=>onAdd(product)} disabled={unavailable}/>;
});
...
export function PosTerminal({products,members,warehouseName}:{products:Product[];members:Member[];warehouseName:string}){
  ...
  useEffect(()=>{if(actionState.status!=="success"||!actionState.saleId||handledSale.current===actionState.saleId)return;handledSale.current=actionState.saleId;setCart([]);setMemberId("");setIdempotencyKey(crypto.randomUUID());router.refresh()},[actionState,router]);

  const matchingProducts=useMemo(()=>{const q=query.trim().toLowerCase();if(!q)return products;return products.filter((product)=>[product.sku,product.barcode,product.name].filter(Boolean).some((value)=>String(value).toLowerCase().includes(q)))},[products,query]);
  const visibleProducts=useMemo(()=>matchingProducts.slice(0,60),[matchingProducts]);
  const {total,totalQty,itemsJson}=useMemo(()=>{let nextTotal=0;let nextQty=0;for(const line of cart){nextTotal+=line.product.sell_amount*line.quantity;nextQty+=line.quantity}return{total:nextTotal,totalQty:nextQty,itemsJson:JSON.stringify(cart.map((line)=>({productId:line.product.id,quantity:line.quantity}))) }},[cart]);

  const addProduct=useCallback((product:Product)=>{if(product.track_stock&&product.stock_qty<=0)return;setCart((current)=>{const existing=current.find((line)=>line.product.id===product.id);if(existing){const nextQuantity=existing.quantity+1;if(product.track_stock&&nextQuantity>product.stock_qty)return current;return current.map((line)=>line.product.id===product.id?{...line,quantity:nextQuantity}:line)}return[...current,{product,quantity:1}]})},[]);
  const changeQuantity=useCallback((productId:string,delta:number)=>{setCart((current)=>current.map((line)=>{if(line.product.id!==productId)return line;const nextQuantity=line.quantity+delta;if(line.product.track_stock&&nextQuantity>line.product.stock_qty)return line;return{...line,quantity:nextQuantity}}).filter((line)=>line.quantity>0))},[]);
  const removeLine=useCallback((productId:string)=>setCart((current)=>current.filter((line)=>line.product.id!==productId)),[]);
