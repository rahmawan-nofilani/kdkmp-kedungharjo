...
export default async function SavingsAccountsPage({searchParams}:PageProps){
 ...
 const pageTitle=intent==="deposit"?"Pilih Rekening untuk Setoran":intent==="withdraw"?"Pilih Rekening untuk Penarikan":"Rekening Simpanan";
 const pageDescription=intent==="deposit"?"Pilih rekening ACTIVE anggota. Setelah dipilih, form setoran resmi akan dibuka dengan seluruh kontrol ledger, kas/bank, dan shift tetap aktif.":intent==="withdraw"?"Pilih rekening ACTIVE anggota. Sistem tetap memeriksa saldo minimum, masa kunci, jatuh tempo, dan aturan produk sebelum penarikan.":"Kelola rekening anggota berdasarkan produk yang telah disetujui. Saldo dan mutasi tetap berasal dari ledger transaksi D1, bukan input manual.";
 return <PageContainer size="full">
  <PageHeader eyebrow="Simpan Pinjam · Simpanan" title={pageTitle} description={pageDescription} actions={<div className={styles.panelHead}><Link href="/simpan-pinjam">Beranda Simpan Pinjam</Link><Link href="/savings/products">Produk</Link><Link href="/savings/reports">Laporan</Link></div>}/>
  {intent?<Alert tone="info" title={intent==="deposit"?"Mode Setoran Masuk":"Mode Penarikan"}>Pilih satu rekening ACTIVE dari daftar. Business logic transaksi tidak berubah.</Alert>:null}
  ...
  {!intent?<section className={styles.hero}><Card className={styles.panel}><div className={styles.panelHead}><div><span>REGISTRY REKENING</span><h3>Pembukaan rekening terkontrol</h3></div><Badge tone="info">LEDGER v11</Badge></div><p>Rekening ACTIVE dapat menerima transaksi sesuai aturan produk. Pembayaran POS dari saldo Simpanan tetap berada di luar boundary runtime saat ini.</p></Card>{canOpen&&canViewMembers?<SavingsAccountOpenPanel members={activeMembers} products={productChoices}/>:null}</section>:null}
