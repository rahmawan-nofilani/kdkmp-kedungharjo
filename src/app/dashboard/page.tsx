            ...
            return (
              <section className="nav-group" key={group.section}>
                ...
                {visibleItems.map((item, index) => (
                  <Link href={item.href} className={`nav-item ${group.section === "Utama" && index === 0 ? "active" : ""}`} key={item.label}>
        ...
        <div className="workspace-content">
          ...
          <section className="dashboard-grid">
            ...
            <article className="panel-card access-panel">
              <div className="panel-heading"><div><span className="panel-label">ACCESS CONTEXT</span><h3>Kewenangan aktif</h3></div></div>
              <div className="permission-list">{access.permissions.map((permission) => <span key={permission}>{permission}</span>)}</div>
            </article>
          </section>
        </div>
      </section>
