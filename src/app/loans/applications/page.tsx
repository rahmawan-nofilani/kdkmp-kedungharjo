    ...
    <div className={`workspace-content ${styles.content}`}>
      <section className={styles.hero}>
        ...
        {canManage ? <details className={styles.create}><summary>+ Buat Pengajuan</summary>{activeMembers.length && choices.length ? <form action={createLoanApplicationAction}>
          ...
          <div className={styles.formGrid}><label>Nominal pengajuan<input name="requested_principal_amount" type="number" min={1} step={1000} required /></label><label>Tenor (bulan)<input name="requested_tenor_months" type="number" min={1} max={360} required /></label></div>
          <label>Tujuan pinjaman<textarea name="purpose" minLength={5} maxLength={500} rows={3} required /></label>
          <div className={styles.formGrid}><label>Penghasilan per bulan<input name="declared_monthly_income_amount" type="number" min={1} step={1000} required /></label><label>Kewajiban berjalan per bulan<input name="declared_monthly_obligation_amount" type="number" min={0} step={1000} defaultValue={0} required /></label></div>
