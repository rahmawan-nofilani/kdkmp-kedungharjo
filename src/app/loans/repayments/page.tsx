...
type ScheduleRow = { contract_id:string;principal_amount:number;interest_amount:number;paid_principal_amount:number;paid_interest_amount:number };
...
export default async function LoanRepaymentsPage({ searchParams }: PageProps) {
  ...
  if (contracts.length) {
    const scheduleResult = await supabase.from("loan_installment_schedule")
      .select("contract_id,principal_amount,interest_amount,paid_principal_amount,paid_interest_amount")
  ...
  for(const row of schedule){
    const outstanding=Math.max(0,Number(row.principal_amount||0)-Number(row.paid_principal_amount||0))
      +Math.max(0,Number(row.interest_amount||0)-Number(row.paid_interest_amount||0));
  ...
  const failure=errorMessage(params.error)||(repaymentResult.error?"Schema angsuran 4E-5 belum tersedia.":null);
  ...
  return <section className="workspace">
    ...
    <div className={`workspace-content ${styles.content}`}>
      <section className={styles.hero}>
        <div><span>PHASE 4E-5 · REPAYMENT CORE</span><h2>Pembayaran masuk dibagi oleh sistem ke bunga lalu pokok, dari periode tertua, dan baru mengubah jadwal setelah jurnal D1 berhasil.</h2><p>Draft pembayaran membekukan allocation snapshot. Saat diposting, D1 mendebit Kas/Bank lalu mengkredit Piutang Pinjaman dan Pendapatan Bunga. Retry memakai idempotency key tetap agar uang yang sama tidak terposting dua kali.</p></div>
        {canPost ? <details className={styles.create}><summary>+ Catat Pembayaran</summary>
          {availableContracts.length&&treasuryAccounts.length?<form action={createLoanRepaymentAction}>
            <label>Kontrak aktif<select name="contract_id" required defaultValue=""><option value="" disabled>Pilih kontrak</option>{availableContracts.map((contract)=>{const member=memberMap.get(contract.member_id);const channels=snapshotChannels(contract.product_snapshot);return <option key={contract.id} value={contract.id}>{contract.contract_number} · {member?.full_name||"Anggota"} · Sisa {money(outstandingMap.get(contract.id)||0)} · {channels.join("/")||"snapshot lama"}</option>;})}</select></label>
      ...
      <section className={styles.metrics}>
        ...
        <article><span>Draft / proses</span><strong>{repayments.filter((row)=>row.status==="DRAFT"||row.status==="PROCESSING").length}</strong><small>harus diselesaikan/retry</small></article>
