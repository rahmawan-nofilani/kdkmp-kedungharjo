...
type ScheduleRow = { id:string;due_date:string;principal_amount:number;interest_amount:number;paid_principal_amount:number;paid_interest_amount:number;status:string };
...
export default async function LoanRepaymentDetailPage({params,searchParams}:PageProps){
  ...
  if(contract){
    const scheduleResult=await supabase.from("loan_installment_schedule").select("id,due_date,principal_amount,interest_amount,paid_principal_amount,paid_interest_amount,status").eq("contract_id",contract.id).eq("organization_id",access.organization.id).order("installment_number",{ascending:true});
    schedule=(scheduleResult.data??[]) as ScheduleRow[];
  }
  const scheduleMap=new Map(schedule.map((item)=>[item.id,item]));
  const outstanding=schedule.reduce((sum,item)=>sum+Math.max(0,Number(item.principal_amount)-Number(item.paid_principal_amount))+Math.max(0,Number(item.interest_amount)-Number(item.paid_interest_amount)),0);
    ...
    <div className={`workspace-content ${styles.content}`}>
      ...
      <section className={styles.detailGrid}>
        <article className={styles.panel}><div className={styles.panelHead}><div><span>PAYMENT SNAPSHOT</span><h3>Nilai pembayaran</h3></div></div><dl className={styles.definition}>
          ...
          <div><dt>Pokok</dt><dd>{money(row.principal_amount)}</dd></div><div><dt>Bunga</dt><dd>{money(row.interest_amount)}</dd></div>
          <div><dt>Denda</dt><dd>{money(row.penalty_amount)}</dd></div><div><dt>Kanal</dt><dd>{row.channel}</dd></div>
          <div><dt>Treasury D1</dt><dd>{row.treasury_account_id}</dd></div><div><dt>Sisa kewajiban kini</dt><dd>{money(outstanding)}</dd></div>
          <div className={styles.wide}><dt>Catatan</dt><dd>{row.request_note||"—"}</dd></div>
        </dl></article>
        <article className={styles.panel}><div className={styles.panelHead}><div><span>EXECUTION</span><h3>D1 & idempotency</h3></div></div><dl className={styles.definition}>
          ...
          <div><dt>D1 Journal</dt><dd>{row.d1_journal_entry_id||"—"}</dd></div><div><dt>Policy alokasi</dt><dd>INTEREST → PRINCIPAL · oldest first</dd></div>
          <div><dt>Event pokok</dt><dd>{row.principal_accounting_event_code}</dd></div><div><dt>Event bunga</dt><dd>{row.interest_accounting_event_code}</dd></div>
      ...
      <section className={styles.panel}><div className={styles.panelHead}><div><span>ALLOCATION SNAPSHOT</span><h3>Pembagian ke jadwal</h3></div><b>{allocations.length}</b></div>
        {allocations.length?<div className={styles.tableWrap}><table><thead><tr><th>Periode</th><th>Jatuh tempo</th><th>Pokok</th><th>Bunga</th><th>Denda</th><th>Status jadwal kini</th></tr></thead><tbody>{allocations.map((allocation)=>{const item=scheduleMap.get(allocation.installment_id);return <tr key={allocation.id}><td><strong>#{allocation.installment_number}</strong></td><td>{item?.due_date||"—"}</td><td>{money(allocation.principal_amount)}</td><td>{money(allocation.interest_amount)}</td><td>{money(allocation.penalty_amount)}</td><td>{item?.status||"—"}</td></tr>;})}</tbody></table></div>:<div className={styles.empty}>Allocation belum tersedia.</div>}
      </section>
      ...
      <section className={styles.notice}><strong>Boundary 4E-5 core</strong><p>Penalty accrual/waiver, reversal pembayaran, reschedule, dan settlement khusus belum dilakukan di halaman ini. Pembayaran posted bersifat append-only; koreksi finansial harus dibuat sebagai transaksi koreksi/reversal terpisah pada fase hardening.</p></section>
    </div>
  </section>;
}
